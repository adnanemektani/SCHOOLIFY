    ...
    <main className="landing-page">
      <section className="hero-section">
        ...
        <div className="hero-content">
          <div className="eyebrow"><span className="status-dot" /> Les compétences de demain, accessibles aujourd’hui</div>
      ...
      <section className="intro-section section-shell"><div className="section-kicker">Notre mission <span>01</span></div><div className="intro-layout"><h2>Le digital change.<br /><em>Tu peux mener le changement.</em></h2><div><p>Schoolify est une E-dTech qui rend les compétences émergentes compréhensibles, pratiques et réellement accessibles. Ici, tu ne collectionnes pas des cours : tu construis une trajectoire.</p><Link className="arrow-link" href="/signup">Faire le premier pas <span>↗</span></Link></div></div><div className="stats-row"><div><strong>92<sup>%</sup></strong><span>de nos apprenants se sentent prêts à appliquer leurs acquis</span></div><div><strong>18</strong><span>modules pratiques pour apprendre à ton rythme</span></div><div><strong>1</strong><span>communauté qui avance à tes côtés</span></div></div></section>

      <section id="programmes" className="tracks-section"><div className="section-shell"><div className="section-kicker">Explorer <span>02</span></div><div className="heading-row"><h2>Choisis le terrain<br />où tu veux <em>briller.</em></h2><p>Des parcours conçus autour des compétences recherchées, avec une vraie place pour la pratique.</p></div><div className="track-grid">{tracks.map((track, index) => <article className={`track-card track-${index + 1}`} key={track.title}><span className="track-number">0{index + 1}</span><div className="track-symbol">{track.icon}</div><h3>{track.title}</h3><p>{track.text}</p><Link href="/signup" aria-label={`Découvrir ${track.title}`}>↗</Link></article>)}</div></div></section>

      <section id="metiers" className="jobs-section section-shell"><div className="jobs-copy"><div className="section-kicker">Des opportunités réelles <span>03</span></div><h2>Ta curiosité<br />peut devenir un <em>métier.</em></h2><p>Le Web3 et l’IA ont besoin de profils divers : créatifs, analytiques, organisés, visionnaires. Peut-être le tien.</p><Link className="button button-outline" href="/signup">Explorer les parcours <span>→</span></Link></div><div className="jobs-list">{jobs.map((job, index) => <div key={job}><span>0{index + 1}</span><strong>{job}</strong><i>↗</i></div>)}</div></section>

      <section id="comment-ca-marche" className="steps-section"><div className="section-shell"><div className="steps-heading"><div className="section-kicker">Simplement <span>04</span></div><h2>De l’envie à<br /><em>l’impact.</em></h2></div><div className="steps-grid"><article><span>01</span><div className="step-icon">⌁</div><h3>Tu crées ton espace</h3><p>Quelques informations, et ton tableau de bord personnalisé est prêt.</p></article><article><span>02</span><div className="step-icon">◉</div><h3>Tu choisis ton rythme</h3><p>Explore, pratique et avance selon ton énergie et tes objectifs.</p></article><article><span>03</span><div className="step-icon">↗</div><h3>Tu passes à l’action</h3><p>Transforme tes nouvelles compétences en projets concrets.</p></article></div></div></section>
